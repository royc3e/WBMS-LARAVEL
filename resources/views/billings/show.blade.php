    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                ...
                <div class="border-t border-gray-200 dark:border-gray-700 px-4 py-5 sm:p-0">
                    <dl class="sm:divide-y sm:divide-gray-200 dark:divide-gray-700">
                            ...
                            <dd class="mt-1 text-sm sm:mt-0 sm:col-span-2">
                                @php
                                    $statusClasses = [
                                        ...
                                        'partial' => 'bg-blue-100 text-blue-800 dark:bg-blue-800 dark:text-blue-100',
                                        ...
                                        'void' => 'bg-gray-100 text-gray-800 dark:bg-gray-600 dark:text-gray-100',
                        ...
                        <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            ...
                            <dd class="mt-1 text-sm font-medium {{ \Carbon\Carbon::parse($billing->due_date)->isPast() && !in_array($billing->status, ['paid', 'void']) ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white' }} sm:mt-0 sm:col-span-2">
                                {{ \Carbon\Carbon::parse($billing->due_date)->format('M d, Y') }}
                                @if(\Carbon\Carbon::parse($billing->due_date)->isPast() && !in_array($billing->status, ['paid', 'void']))
        ...
        <div class="space-y-6">
                ...
                <div class="p-6">
                    ...
                    <div class="mt-6 space-y-4">
                        ...
                        <div class="pt-4 mt-4 border-t border-gray-200 dark:border-gray-700">
                            ...
                            @else
                                <a href="{{ route('billings.payments.create', $billing) }}" class="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                    Process Payment
                                </a>
                            @endif
                        </div>
            ...
            <div class="bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-lg">
                ...
                <div class="p-4 space-y-4">                     @if($billing->status !== 'void')
                        <a href="{{ route('billings.payments.create', $billing) }}" class="w-full flex justify-center items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                            <svg class="-ml-1 mr-2 h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                        ...
                        @if($billing->status === 'pending' || $billing->status === 'overdue' || $billing->status === 'partial')
                            <form action="{{ route('billings.cancel', $billing) }}" method="POST" onsubmit="return confirm('Are you sure you want to void this billing? This action cannot be undone.');">
                                @csrf
                                @method('PATCH')
